...
/**
 *Tells you where the closest road point is
 * */
...
/*
 * It can indicate given the direction you are headed, what is the next way
 * point
 */
...
//          ptsy.push_back(way_pt_3[1]);
          ...
          std::vector<std::tuple<double, double>> pts_sorted;
          for (int i = 0; i < ptsx.size(); i++) {
            pts_sorted.push_back(std::make_pair(ptsx[i], ptsy[i]));
          }
          ...
          // Add previous points (remaining ones)-- so that transition is smooth
          // The simulator is going through some points
          // and we eliminate those points
          ...
          for (int i = 1; i <= 30 - previous_path_x.size(); i++) {
            ...
            double x_ref = x_pt;
            double y_ref = y_pt;
            // Transform from car local to global x,y
          ...
          for (int i = 0; i < 50; i++) {
            ...
            double next_d = 6;
            vector<double> Xxy = getXY(next_s, next_d, map_waypoints_s,
                                       map_waypoints_x, map_waypoints_y);
            next_x_vals.push_back(Xxy[0]);
            next_y_vals.push_back(Xxy[1]);
          } */
